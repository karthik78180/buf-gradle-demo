...
**What it is:**
- Auto-generated lock file that pins exact versions of proto dependencies
- Records commit hashes and checksums from `buf.yaml` deps

**When it's generated:**
- Automatically created/updated during Gradle build
- Generated by `build.buf` Gradle plugin when resolving dependencies

**Gradle tasks that trigger buf.lock generation:**
```bash
./gradlew build          # Full build
./gradlew bufLint        # Linting
./gradlew bufValidate    # Validation
./gradlew generateProto  # Proto generation
```
